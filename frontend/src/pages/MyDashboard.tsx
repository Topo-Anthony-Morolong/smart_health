...
export function MyDashboard() {
  ...
  return (
    <div className="p-6 md:p-10 max-w-3xl mx-auto space-y-6">
          ...
          <p className="text-slate-500 text-sm mt-1">
            {patient.medical_history && (
              <span className="truncate max-w-xs block">
                {patient.medical_history.slice(0, 60)}
                {patient.medical_history.length > 60 ? "…" : ""}
              </span>
            )}
      ...
      {alerts.length > 0 && (
        <div className="bg-red-50 border-2 border-red-200 rounded-2xl p-4 space-y-2">
          <div className="flex items-center gap-2 text-red-700 font-bold">
            <Bell className="h-4 w-4" /> {alerts.length} Unacknowledged Alert
            {alerts.length > 1 ? "s" : ""}
          </div>
          {alerts.map((a) => (
            <div
              key={a.id}
              className="flex items-start justify-between gap-3 bg-white rounded-xl p-3 border border-red-100"
            >
              <div className="flex items-center gap-2">
                <AlertTriangle
                  className={`h-4 w-4 shrink-0 ${a.severity === "critical" ? "text-red-500" : "text-amber-500"}`}
                />
      ...
      {/* Latest Risk Card */}
      <div
        className={`rounded-3xl border-2 p-6 relative overflow-hidden ${riskLevel === "high"
      ...
      >
        <div className="absolute right-6 top-6 opacity-10">
          <Heart
            className={`h-28 w-28 ${riskLevel === "high" ? "text-red-600" : riskLevel === "medium" ? "text-amber-500" : "text-emerald-600"}`}
          />
        </div>
        ...
        {latest ? (
          <>
            <div className="flex items-center gap-3 mb-3">
              <TrendIcon
                className={`h-7 w-7 ${riskLevel === "high" ? "text-red-600" : riskLevel === "medium" ? "text-amber-600" : "text-emerald-600"}`}
              />
              <span
                className={`text-4xl font-extrabold ${riskLevel === "high" ? "text-red-700" : riskLevel === "medium" ? "text-amber-700" : "text-emerald-700"}`}
              >
                {cfg.label}
              </span>
            </div>
            ...
            <div className="mb-3">
              ...
              <div className="h-3 bg-white/70 rounded-full overflow-hidden border border-slate-200">
                <div
                  className={`h-full ${cfg.bar} rounded-full transition-all duration-700 ${getWidthClass(latest.risk_score)}`}
                />
              </div>
            </div>
            <p className="text-xs text-slate-500">
              Recorded{" "}
              {format(parseISO(latest.recorded_at), "MMM d, yyyy · HH:mm")}
            </p>
          </>
        ) : (
          <div className="flex items-center gap-3">
            <p className="text-slate-500 text-base">
              No readings yet. Log your first vitals to see your risk score.
            </p>
          </div>
        )}
      </div>
            ...
            {history.map((r) => {
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`h-8 w-8 rounded-xl flex items-center justify-center border text-xs font-bold ${c.badge}`}
                    >
                      {r.risk_score.toFixed(0)}
                    </div>
                    <div>
                      <p className="text-xs text-slate-400">
                        {format(parseISO(r.recorded_at), "MMM d, yyyy")}
                      </p>
                      <p className="text-sm font-medium text-slate-700">
                        HR {r.heart_rate} · BP {r.blood_pressure_systolic}/
                        {r.blood_pressure_diastolic} · SpO₂{" "}
                        {r.oxygen_saturation}%
                      </p>
                    </div>
                  </div>
                  <span
                    className={`hidden sm:inline-flex px-2.5 py-1 rounded-full text-xs font-bold border ${c.badge}`}
                  >
                    {lvl}
                  </span>
                </li>
