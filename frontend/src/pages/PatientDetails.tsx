...
export function PatientDetails() {
    ...
    async (pid: string) => {
      ...
      try {
        ...
        setEditForm({
          ...
          gender: pt.gender,
          contact: pt.contact ?? "",
          medical_history: pt.medical_history ?? "",
        });
      } catch {
        navigate("/patients");
      } finally {
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto space-y-6 bg-slate-50 min-h-full">
      ...
      <div className="flex items-center justify-between pb-4 border-b border-slate-200">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 text-sm font-semibold text-slate-600 bg-white border border-slate-200 rounded-xl shadow-sm hover:bg-slate-50 transition-colors"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 text-sm font-semibold text-white bg-red-500 border border-red-500 rounded-xl shadow-sm hover:bg-red-600 transition-colors"
          >
            <Trash2 className="h-4 w-4" /> Delete
          </button>
          <button
            onClick={() => setVitalOpen(true)}
            className="flex items-center gap-1.5 px-3 py-2 text-sm font-semibold text-white bg-gradient-to-r from-blue-600 to-teal-500 rounded-xl shadow-sm hover:opacity-90 transition-opacity"
          >
      ...
      {vitalResult && (
        <div
          className={`rounded-2xl border-2 p-4 ${vitalResult.alert_triggered ? "border-red-300 bg-red-50" : "border-emerald-300 bg-emerald-50"}`}
        >
          <div className="flex items-center gap-2 font-bold mb-1">
            {vitalResult.alert_triggered ? (
            ...
            >
              {vitalResult.risk_level} Risk · Score{" "}
              {vitalResult.risk_score.toFixed(0)}
            </span>
            {vitalResult.alert_triggered && (
          ...
          <ul className="list-disc pl-5 text-sm space-y-0.5">
            {vitalResult.recommendations.map((r, i) => (
              <li
                key={i}
                className={
                  vitalResult.alert_triggered
      ...
      <div className="flex gap-1 bg-slate-100 p-1 rounded-xl w-fit">
        <button
          ...
          className={`px-5 py-2 rounded-lg text-sm font-semibold transition-colors flex items-center gap-2 ${tab === "vitals" ? "bg-white text-blue-700 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
        >
          <Activity className="h-4 w-4" /> Vitals
        </button>
        <button
          onClick={() => setTab("alerts")}
          className={`px-5 py-2 rounded-lg text-sm font-semibold transition-colors flex items-center gap-2 ${tab === "alerts" ? "bg-white text-blue-700 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
        >
          ...
          {history.length === 0 ? (
            <div className="py-16 text-center text-slate-400">
              ...
              <p className="font-medium">
                No vitals recorded yet. Click "Log Vitals" to begin.
              </p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left">
                <thead className="bg-slate-50 text-xs font-semibold text-slate-500 uppercase border-b border-slate-100">
                  <tr>
                    {["Date", "HR", "BP", "Temp", "SpO₂", "Score", "Risk"].map(
                      (h) => (
                  ...
                  {history.map((r) => (
                    ...
                    >
                      <td className="px-5 py-3.5 text-slate-500 whitespace-nowrap">
                        {format(parseISO(r.recorded_at), "MMM d, yyyy HH:mm")}
                      </td>
                      ...
                      <td className="px-5 py-3.5">
                        <span
                          className={`px-2.5 py-1 rounded-full text-xs font-bold border ${RISK_STYLE[r.risk_level ?? "low"]}`}
                        >
                          {r.risk_level}
                        </span>
                      </td>
          ...
          {alerts.length === 0 ? (
            ...
            <ul className="divide-y divide-slate-50">
              {alerts.map((a) => (
                <li
                  key={a.id}
                  className={`p-4 flex items-start gap-4 ${a.is_acknowledged ? "opacity-60" : ""} hover:bg-slate-50 transition-colors`}
                >
                  <div
                    className={`p-2 rounded-xl border shrink-0 ${a.severity === "critical" ? "bg-red-50 border-red-200" : "bg-amber-50 border-amber-200"}`}
                  >
                    <AlertTriangle
                      className={`h-4 w-4 ${a.severity === "critical" ? "text-red-500" : "text-amber-500"}`}
                    />
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <span
                        className={`text-xs font-bold px-2 py-0.5 rounded-full border ${a.severity === "critical" ? "bg-red-50 text-red-700 border-red-200" : "bg-amber-50 text-amber-700 border-amber-200"}`}
                      >
                        {a.severity}
                      </span>
                      {a.is_acknowledged && (
        ...
        <ModalOverlay onClose={() => setEditOpen(false)}>
          ...
          <div className="space-y-4">
            {(
              ...
              ] as [keyof PatientUpdate, string, string][]
          ...
          <div className="flex justify-end gap-3 mt-6">
            ...
            <button
              ...
              disabled={editSaving}
              className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white bg-blue-600 rounded-xl hover:bg-blue-700 transition-colors disabled:opacity-60"
            >
              {editSaving ? (
        ...
        <ModalOverlay onClose={() => setDeleteOpen(false)}>
          <div className="text-center">
            ...
            <p className="text-slate-500 mt-2 text-sm">
              This will permanently delete <strong>{patient.name}</strong> and
              all associated readings and alerts. This action cannot be undone.
            </p>
            <div className="flex justify-center gap-3 mt-6">
              ...
              <button
                ...
                className="flex items-center gap-2 px-5 py-2.5 text-sm font-semibold text-white bg-red-500 rounded-xl hover:bg-red-600 transition-colors disabled:opacity-60"
              >
                {deleteLoading ? (
                  <Loader2 className="h-3.5 w-3.5 animate-spin" />
                ) : (
      ...
      {vitalOpen && (
        <ModalOverlay onClose={() => setVitalOpen(false)}>
          ...
          <p className="text-sm text-slate-500 mb-5">
            Results are analyzed by the ML risk engine instantly.
          </p>
          <form onSubmit={handleVitalSubmit} className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2 flex justify-end gap-3 mt-2">
              ...
              <button
                type="submit"
                disabled={submitting}
                className="flex items-center gap-2 px-5 py-2 text-sm font-semibold text-white bg-gradient-to-r from-blue-600 to-teal-500 rounded-xl hover:opacity-90 disabled:opacity-60"
              >
                {submitting ? (
                  <Loader2 className="h-3.5 w-3.5 animate-spin" />
                ) : (
...
function ModalOverlay({
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 relative border border-slate-200 ring-1 ring-slate-900/5"
